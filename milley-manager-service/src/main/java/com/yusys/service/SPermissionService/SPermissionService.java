...
public class SPermissionService implements ISPermissionService {
	...
	public Map<String, String> addUserPermiss(HttpServletRequest req, String userid) throws Exception{ 
		...
		if ("byuser".equals(crtype)) {
			...
			issucess=startCreateByRole(RequestUtils.getParamValue(req, "crobj"),userid);
	...
	private String getMenuActions(List<Map<String, String>> userRoles){
		...
		for (int i = 0; i < roleMenuActions.size(); i++) {
			...
			Integer count=actionCount.get(roleMenuActions.get(i).get("MENU_NO")+"_"+roleMenuActions.get(i).get("ACTION_NO"));
			if (count==null) {
				count=0;
			}
			count++;
			actionCount.put(roleMenuActions.get(i).get("MENU_NO")+"_"+roleMenuActions.get(i).get("ACTION_NO"),count);//设置按钮出现的 次数
		...
		for (int i = 0; i < menunos.size(); i++) {
			...
			for (int j = actions.size()-1; j >=0 ; j--) {
				Integer c=actionCount.get(menunos.get(i)+"_"+actions.get(j));
