...
public class SDicService implements ISDicService{
	...
	public Map<String, String> save(HttpServletRequest req,String userid) {
		...
		try {
			//必填参数列表
			String[] must=new String[]{"dic_code","dic_name","state"};
			//非必填的参数列表
			String[] nomust=new String[]{"memo","menu_no"};
...
//			String[] nomust=new String[]{"supmenu_no","memo"};
	...
	public Map<String, Object> findAll(HttpServletRequest req,String userid) {
		...
		try {
			...
			String dic_name=req.getParameter("dic_name");
			String dic_code=req.getParameter("dic_code");
			if(dic_name!=null){
			...
			pmap.put("state", req.getParameter("state"));
	...
	@Override
	public List<Map<String, String>> findItemByDic(HttpServletRequest req,String userid) {
		String []must={"dic_code"};
		Map<String, String> pmap=RequestUtils.requestToMap(req, must, null);
		try {
			if (pmap!=null) {
				List<Map<String, String>> listMap=null;//redisCache.get(pmap.get("dic_code")+IMyCache.dic, null);
				if (listMap==null||listMap.size()==0) {
					return sdicdao.findItemByDic(pmap);
				}
				return listMap;
			}
		} catch (Exception e) {
	...
	public void refreshDicItemToRedis(String dic_code){
		if (dic_code!=null) {
			Map<String, String> pmap=new HashMap<String, String>();
			pmap.put("dic_code", dic_code);
			//redisCache.put(dic_code+IMyCache.dic, sdicdao.findItemByDic(pmap));
		}
	}
}
