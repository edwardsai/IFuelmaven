...
import com.yusys.workFlow.WFEntryDao;
...
@Service(value="WF14")
@Transactional
public class EmpEntryService extends WFAppBizBaseService {
	@Resource
	private WFEntryDao wfEntryDao;
	@Resource
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return wfEntryDao.queryWFEntryBizData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		Map<String, String> emp_map=wfEntryDao.queryWFEntryBizData(bizId);
		Map<String,String> empInfo = new HashMap<String, String>();//存人员信息
		//获取当前日期"yyyy-MM-dd"
		String today = DateTimeUtils.getFormatCurrentDate();
		map.put("approve_status", "997");//通过状态
		map.put("id", bizId);//人员信息id
		empInfo = wfEntryDao.queryEmpinfobyid(map);
		System.out.println(empInfo);
		emp_map.put("EMP_STATUS", "02");//在场状态
		emp_map.put("ENTER_DATE", today);//进场时间
		System.out.println("222222222222");
		emp_map.put("EMP_POST", empInfo.get("EMP_POST"));
		System.out.println(emp_map.get("EMP_POST"));
		emp_map.put("EMP_GRADE", empInfo.get("EMP_GRADE"));
		System.out.println(emp_map.get("EMP_GRADE"));
		//通过人员id获取人员编号，并通过编号修改人员基本信息表的状态为在场状态
		try {
			//更新人员基本信息状态
			wfEntryDao.updateEmpStaByNum(emp_map);
			//更新人员入场信息状态
			wfEntryDao.updateEntranceAppStaById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfReject(String instanceId, String bizId) {
		//流程被打回时，审批状态改为打回状态
		Map<String, String> map=new HashMap<String, String>();
		map.put("approve_status", "992");//打回状态
		map.put("id", bizId);//人员信息id
		try {
			//更新人员信息状态
			wfEntryDao.updateEntranceAppStaById(map);
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = wfEntryDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				wfEntryDao.updateEmpAppPersonById(map);
				return;
			}
		}
	}
