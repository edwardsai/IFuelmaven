...
import com.yusys.workFlow.WFOutDao;
...
@Service(value="WF61")
@Transactional
public class EmpOutService extends WFAppBizBaseService {
	@Resource
	private WFOutDao wfOutDao;
	@Resource
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return wfOutDao.queryWFOutBizData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		Map<String, String> emp_map=wfOutDao.queryWFOutBizData(bizId);
		//获取当前日期"yyyy-MM-dd"
		String today = DateTimeUtils.getFormatCurrentDate();
		map.put("approve_status", "997");//通过状态
		map.put("id", bizId);//人员信息id
		emp_map.put("EMP_STATUS", "03");//离场状态
		emp_map.put("LEAVE_DATE", today);//离场状态
		
		//通过人员id获取人员编号，并通过编号修改人员基本信息表的状态为在场状态
		try {
			//更新人员基本信息状态（离场），离场时间为当前时间
			wfOutDao.updateEmpStaByNum(emp_map);
			//更新人员离场信息状态（通过）
			wfOutDao.updateOutAppStaById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfPass(String instanceId, String bizId) {
		...
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				wfOutDao.updateEmpAppPersonById(map);
				return;
			}
		}
	}
