...
import com.yusys.workFlow.WFProjectDao;

@Service(value="WF82")
@Transactional
public class ProjectevaluateService extends WFAppBizBaseService {
	@Resource
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return wfProjectDao.queryWFProjectData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		map.put("audit_status", "03");//完成审核状态
		map.put("pro_eva_plan_num", bizId);
		try {
			//更新审批状态
			wfProjectDao.updateProjectById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = wfProjectDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				wfProjectDao.updateProPersonById(map);
				return;
			}
		}
	}
					...
					if(bizIdArr[j]!=null&&!"".equals(bizIdArr)){
						for(int i=0;i<psArr.length;i++){
							if(psArr[i]!=null&&!"".equals(psArr[i])){
								...
								map.put("pro_eva_plan_num", bizIdArr[j]);
								wfProjectDao.updateProPersonById(map);
							}
						}
					}
