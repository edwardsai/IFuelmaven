...
import com.yusys.workFlow.WFEmpEvaDao;

@Service(value="WF202")
@Transactional
public class EmpEvaService extends WFAppBizBaseService{
	@Resource
	private WFEmpEvaDao wfEmpEvaDao;
	@Resource
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return wfEmpEvaDao.queryWFEmpData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		map.put("plan_id", bizId);
		try {
			//更新审批状态
			wfEmpEvaDao.updateEmpById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = wfEmpEvaDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				wfEmpEvaDao.updateEmpPersonById(map);
				return;
			}
		}
	}
