...
@Service(value="WF81")
@Transactional
public class BgrChkMgrSubmitChkService extends WFAppBizBaseService {
	@Resource
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return bgrChkMgrSubmitChkDao.queryWFBgrChkPlanBizData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		map.put("approval_status", "03");//通过状态
		map.put("id", bizId);//计划id
		try {
			//更新计划信息状态
			bgrChkMgrSubmitChkDao.updateBgrChkPlanSttById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfReject(String instanceId, String bizId) {
		//流程被打回时，审批状态改为打回状态
		Map<String, String> map=new HashMap<String, String>();
		map.put("approval_status", "05");//打回状态
		map.put("id", bizId);//计划id
		try {
			//更新状态
			bgrChkMgrSubmitChkDao.updateBgrChkPlanSttById(map);
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = bgrChkMgrSubmitChkDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				bgrChkMgrSubmitChkDao.updateBgrReportChkPersonById(map);
				return;
			}
		}		
