...
import com.yusys.workFlow.WFEmpAskForLeaveDao;
...
@Service(value="WF101")
@Transactional
public class EmpAskForLeaveService extends WFAppBizBaseService {
	@Resource
	private WFEmpAskForLeaveDao empAskForLeaveDao;
	/**
	 * 获取节点审批人 
	 */
	@Override
	public List<Map<String, String>> getNodeAppPerson(String bizId,String nid) {
		...
		String rulerExp=empAskForLeaveDao.queryRulerExpByNodeId(nid);//根据节点找到规则表达式
		String roleGroup="";
		//循环取出角色编号
		...
		String[] userNo = empAskForLeaveDao.queryAllUserNo(roleNoMap);//获取人员编号
		Map<String, Object> userNoMap=new HashMap<String, Object>();
		userNoMap.put("userNo", userNo);
		List<Map<String, String>> umap = empAskForLeaveDao.queryAllUser(userNoMap);//获取人员信息
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return empAskForLeaveDao.queryWFAskForLeaveBizData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		map.put("status", "997");//通过状态
		map.put("id", bizId);//人员id
		try {
			//更新计划信息状态
			empAskForLeaveDao.updateAskForLeaveSttById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = empAskForLeaveDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				empAskForLeaveDao.updateAskForLeaveAppPersonById(map);
				return;
			}
		}		
	}
	//获取第一个审批人
	@Override
	public Map<String, String> insertNextAppPerson(String wf_id, String bizId,
			String ps) {
