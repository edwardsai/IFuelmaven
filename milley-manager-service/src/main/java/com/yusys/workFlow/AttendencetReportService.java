...
import com.yusys.workFlow.WFAttendenceDao;
import com.yusys.workFlow.WFBgrChkMgrSubmitChkDao;

@Service(value="WF203")
@Transactional
public class AttendencetReportService extends WFAppBizBaseService {
	/*@Resource
	...
	@Resource
	private WFAttendenceDao wfAttendenceDao;
	/**
	 * 获取节点审批人 
	 */
	@Override
	...
	public Map<String, String> getWFAppBizData(String bizId) {
		return wfAttendenceDao.queryWFAttendenceData(bizId);
	}
	/**
	 * 当流程走完时 执行的一个操作
	 */
	...
	public void wfSuccess(String instanceId, String bizId) {
		...
		try {
			//更新审批状态
			wfAttendenceDao.updateAttendenceById(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void wfPass(String instanceId, String bizId) {
		...
		List<Map<String, Object>> list = wfAttendenceDao.queryAppIdByInstId(wfmap);
		if(list.size()>0){
			for(Map<String, Object> map1:list){
				...
				//设置当前审批人
				wfAttendenceDao.updateCurrPersonById(map);
				return;
			}
		}
	}
