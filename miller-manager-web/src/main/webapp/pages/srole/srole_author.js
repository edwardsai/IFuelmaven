...
function initAuthTables(menu_no){
	$('#dataButton').bootstrapTable('destroy').bootstrapTable({
		url : 'SDicItem/findAllSDicItem.asp?dic_code=6', //请求后台的URL（*）
		method : 'get', //请求方式（*）   
		striped : true, //是否显示行间隔色
		cache : false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		pagination : false, //是否显示分页（*）
		...
		columns : [ {
			...
			var sroleInfo=$("#nowSRole").text();
			$.post("SRole/DataAuthChecked.asp",{"menu_no":menu_no,"role_no":sroleInfo},function(msg){
				if(msg==undefined){
					return;
				}
				for(var i=0;i<data.rows.length;i++){
...
function initRoleMenuTree() {
	...
	setting.callback.onAsyncSuccess=function(){
		sroleMenuZTreeObj.checkAllNodes(false);
		var sroleInfo=$("#nowSRole").text();
		$.ajax({
			type:"post",
	...
	sroleMenuZTreeObj = $.fn.zTree.getZTreeObj("treeMenu");	
...
function getCheckedAll(){		
    var nodes = sroleMenuZTreeObj.getCheckedNodes(true);
...
//保存菜单节点值
$("#saveSRolem").click(function(){	
	var memunos=getCheckedAll();
	var sroleInfo=$("#nowSRole").text();				
	$.ajax({
		type : "post",
