...
<div id="test1" style="width:100%;height:auto;">
	<form id = "taskDetailReportForm" action=""  class="form-inline">
		<div class="row">
			<div class="span6 h50">
				<label class="pl45" style="float:left;">任务状态：</label>
				<select id="task_state"  name="T.task_state"  style="width:223px;margin-left:0px;"   diccode="T_DIC_STATE"></select>
			</div>
			<div class="span6 h50">
				<label class="pl45" style="float:left;">任务类型：</label> 
				<select id="task_type"  name="T.task_type" style="width:223px;margin-left: 0px;" diccode="T_DIC_TYPE"></select>
			</div>
		</div>
		<div class="row">
			<div class="span6 h50">
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#query").click(function(){
			...
			$.ajax({
				type : "post",
				url : "TaskChangeInfo/updateTaskState.asp?task_id="+ids,
				dataType : "json",
			...
			*/
			//alert('<%=request.getParameter("currentUserId")%>' +'----'+$("#task_executor").val()+'----'+$("#execute_dept").val()+'----'+$("#task_type").val()+'----'+$("#task_state").val());
			report.init(); //初始化报表控件
			report.setFile("detail_report.raq");//设置要显示的报表文件   
			report.setSaveAsName("任务明细表");
			report.putParam("org_no","<%=request.getParameter("currentOrgNo")%>");
			...
			report.putParam("task_type",$("#task_type").val());
			report.putParam("task_state",$("#task_state").val());
