function initViewAssetTransferDept(params){
	...
	for(var k in params){
		...
		if (k1=="approve_remark") {
			...
			currTab.find("span[name='PTD."+k1+"']").html(params[k]);
		}
	}
	initDeptAssetList(params.TRANSFER_ID);//初始化转移列表
	
	//返回按钮
	currTab.find("#backToDeptTransferMain").click(function(){
...
/****************************************************************************************/
	function initDeptAssetList(transfer_id){
		...
		currTab.find("#tb_propertyTransf_DetailsList").bootstrapTable('destroy').bootstrapTable({
