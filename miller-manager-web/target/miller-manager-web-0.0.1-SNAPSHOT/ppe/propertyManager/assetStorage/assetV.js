...
function initPersonalAssetEditLayout(item){
	...
	var page = $("#personalAssetStorageEditPart1");
	page.find("select,input,textarea").attr("disabled","disabled");
	initVlidate($("#personalAssetStorageEditPart1"));
	if(item){
		for (var key in item) {
			page.find("input[name="+"V"+key+"]").val(item[key]);
		}
	...
	var rowNum = 2;
	var assetType = $.trim(page.find("input[name=VassetType]").val());
		...
		$.getJSON("propertyFieldConfig/queryAllPrivateField.asp?category_id="+assetType,function(priFieldData){
			...
			if(item){
				$.getJSON("inventoryManager/assetStorage/querySubAssetStorage.asp", {serialsNumber:item.serialsNumber}, function(data){
					fieldData = data;
					var inputssss = table.find("input");
	...
	back.click(function(){
		newOpenTab("assetstorage","资产入库","ppe/propertyManager/assetStorage/assetList.html");
	});
	//生成动态类型表格HTML代码
	function getRowLine(item, isPub){
		...
		if(item.TAG_TYPE=="文本框"){
			...
			if (item.DEFAULT_VALUE==null) {
				item.DEFAULT_VALUE="";
				str += "<td><select style='width:344px;' type='text' maxlength='"+item.MAX_LENGTH+"' col_code='"+item.COL_ID+"' diccode='"+item.DIC_CODE+"' name='"+item.CATEGORY_NAME+"' is_common='"+isPub+"' multiple='multiple' value='"+item.DEFAULT_VALUE+"'></select></td>";
